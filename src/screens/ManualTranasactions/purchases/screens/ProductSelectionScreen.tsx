...
const ProductSelectionScreen: React.FC = () => {
  ...
  // Get supplier from route params or Redux
  // @ts-ignore
  const supplierFromParams = route.params?.supplier;
  const supplierFromRedux = useAppSelector(selectSelectedSupplier);
  const supplier = supplierFromParams || supplierFromRedux;

  const products = useAppSelector(selectRecentProducts);
  const cartItems = useAppSelector(selectPurchaseItems);
  const productsLoading = useAppSelector((state) => state.purchases.productsLoading);
  ...
  const handleProductSelect = useCallback(
    (product: Product) => {
      // @ts-ignore
      navigation.navigate('PurchaseQuantityPrice', { product, supplier });
    },
    [navigation, supplier]
  );
  ...
  const handleSkipToTotal = useCallback(() => {
    ...
    // @ts-ignore
    navigation.navigate('DirectTotal', { supplier });
  }, [dispatch, navigation, supplier]);

  const handleViewCart = useCallback(() => {
    // @ts-ignore
    navigation.navigate('PurchaseBillSummary', { supplier });
  }, [navigation, supplier]);
  ...
  const isLowStock = (product: Product): boolean => {
    return product.min_stock_level !== undefined && product.current_stock <= product.min_stock_level;
  };
    ...
    ({ item }: { item: Product }) => {
      ...
      return (
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <View style={[styles.productIcon, lowStock && styles.productIconLowStock]}>
            <PackageIcon
              size={24}
          ...
          <View style={styles.productInfo}>
            ...
            <View style={styles.priceRow}>
              <Text style={styles.productPrice}>
                PKR {formatCurrency(item.purchase_price)}/{item.unit}
              </Text>
              {lowStock && (
