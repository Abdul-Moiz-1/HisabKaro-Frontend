...
const CustomerLedgerScreen: React.FC<NavigationProps<'CustomerLedger'>> = ({
  navigation,
  route,
}) => {
  ...
  const fetchData = useCallback(
    async (isRefresh: boolean = false) => {
      ...
      try {
        ...
        const [customerData, statementData] = await Promise.all([
          customersApi.getById(customerId),
          customersApi.getStatement(customerId, startDate, endDate),
        ]);
    ...
    [customerId, filterPeriod, getDateRange],
  );
  ...
  const handleWhatsAppShare = async () => {
    try {
      const message =
        ...
        `Total Sales: Rs. ${customer?.totalSales?.toLocaleString() || 0}\n` +
        `Total Payments: Rs. ${
          customer?.totalPayments?.toLocaleString() || 0
        }\n\n` +
  ...
  const getRunningBalance = (index: number): number => {
    let balance = 0;
    // Calculate from the end to start (oldest to newest)
    for (let i = statements.length - 1; i >= index; i--) {
      balance += statements[i].debit - statements[i].credit;
    }
    return balance;
  };
      ...
      >
        ...
        <View style={styles.profileSection}>
          ...
          <Text style={styles.balanceLabel}>
            {isUrdu ? 'باقی بیلنس (وصولی)' : 'BAAQI BALANCE (RECEIVABLE)'}
          </Text>
        </View>
