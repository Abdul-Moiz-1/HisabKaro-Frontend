...
const PurchaseInvoiceListScreen: React.FC = () => {
  ...
  const fetchInvoices = useCallback(
    async (pageNum: number = 1, refresh: boolean = false) => {
      try {
        ...
        const response = await purchaseInvoicesApi.getAll(filters);
  ...
  const handleInvoicePress = useCallback(
    (invoice: Invoice) => {
      // @ts-ignore
      navigation.navigate(ROUTES.PURCHASE_INVOICE_DETAIL, {
      ...
      return (
        <TouchableOpacity
          ...
          <Text style={styles.supplierName}>
            {item.supplierName || 'Walk-in Supplier'}
          </Text>
  ...
  const renderEmpty = () => {
    ...
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.emptyTitle}>No Purchase Bills</Text>
        <Text style={styles.emptySubtitle}>
          Your purchase bills will appear here
        </Text>
      </View>
    );
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={styles.headerSubtitle}>Kharidari Bills</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
