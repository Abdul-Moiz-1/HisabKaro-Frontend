...
const SalesInvoiceDetailScreen: React.FC = () => {
  ...
  const fetchInvoiceDetail = useCallback(async () => {
    try {
      setLoading(true);
      const invoiceRes = await salesInvoicesApi.getById(invoiceId);
      const invoiceData = (invoiceRes as any)?.data ?? invoiceRes;
      setInvoice(invoiceData);
      setGlEntries((invoiceRes as any)?.glEntries || invoiceData?.glEntries || []);

      // The detail response includes payments inline; fall back to the dedicated endpoint
      let paymentsList: InvoicePayment[] = [];
      if (Array.isArray(invoiceData?.payments) && invoiceData.payments.length > 0) {
        paymentsList = invoiceData.payments;
      } else {
        try {
          const paymentsRes = await salesInvoicesApi.getPayments(invoiceId);
          const raw = Array.isArray(paymentsRes)
            ? paymentsRes
      ...
      <ScrollView
        ...
        <View style={styles.card}>
          ...
          <View style={styles.amountsRow}>
            ...
            <View style={styles.amountBox}>
              <Text style={styles.amountBoxLabel}>Outstanding</Text>
...
export default SalesInvoiceDetailScreen;
